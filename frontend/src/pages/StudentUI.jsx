...
const StudentUI = () => {
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      const response = await api.post('/orders', { item: 'Spaghetti Monolith' });
...
//   }, 1000);
  ...
  const steps = [
    ...
    { id: 'verified', label: 'Stock Verified', icon: <Package size={20} /> },
    { id: 'kitchen', label: 'In Kitchen', icon: <ChefHat size={20} /> },
        ...
        <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <button 
            ...
            className={`px-8 py-4 rounded-2xl font-bold transition-all ${
              orderStatus ? 'bg-slate-100 text-slate-400' : 'bg-indigo-600 text-white shadow-lg shadow-indigo-200 hover:scale-105 active:scale-95'
            }`}
          >
            {loading ? <Loader2 className="animate-spin" /> : orderStatus ? 'Order Placed' : 'Order Now'}
          </button>
        </div>
              ...
              {steps.map((step, index) => {
                ...
                return (
                  <div key={step.id} className="relative z-10 flex flex-col items-center gap-3">
                    ...
                    <span className={`text-sm font-semibold ${isCompleted ? 'text-indigo-600' : 'text-slate-300'}`}>
                      {step.label}
                    </span>
                  </div>
                );
